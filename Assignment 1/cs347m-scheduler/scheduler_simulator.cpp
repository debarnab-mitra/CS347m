...
int main()
{
	...
	//cout << "curtime = " << curtime << endl;
		...
		//routine for handling process admission event
			...
			//cout << "timeslice = " << p_levels[next.pid] << " pid = " << next.pid <<  endl;///////////////////////
					runn=next.pid;
					prior=next.priority;
					ld=curtime;
			...
			//cout << p_unrolled[next.pid][p_ptrs[next.pid]].first << " " << p_ptrs[next.pid] << endl;
					break;
				}
				case 4:
				{	
				...
				// for demotion
						...
						if (flag) {
							...
							cout<<"PID :: "<<next.pid<<"  TIME :: "<<next.end_t<<"  EVENT :: "<<"Priority demoted to level " << new_prior <<endl;
						}
				...
				// for promotion
						...
						if (flag) {
							em.p_priorities[next.pid] = new_prior;
							p_levels[next.pid] = new_lvl;
							cout<<"PID :: "<<next.pid<<"  TIME :: "<<next.end_t<<"  EVENT :: "<<"Priority promoted to level " << new_prior <<endl;
						}
					}
...
//					em.event_table.insert(event(next.end_t, 10, -1, 100));
		...
		int timeslice = Scheduler.levels_arr.begin()->time_slice;
		while(!em.is_empty()){
			event next;
			next = em.next_event();
			curtime = next.end_t;
